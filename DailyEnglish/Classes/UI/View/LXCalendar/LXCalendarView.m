...
-(void)responData :(BOOL)needSelectToday{
    ...
    for (int i = 0; i <42; i++) {
        ...
        if (i >= firstWeekday && i < (firstWeekday + totalDays)) {
            ...
            for(int j=0; j<self.datas.count; j++){
                NSDictionary *dict = [self.datas objectAtIndex:j];
                
                NSString *singInData = [dict objectForKey:@"time"];
                
                NSArray *arr = [singInData componentsSeparatedByString:@"-"];
                
                if(model.year == [[arr objectAtIndex:0] integerValue] && model.month == [[arr objectAtIndex:1] integerValue] && model.day == [[arr objectAtIndex:2] integerValue]){
                    model.isSingIn = YES;
                    break;
                }
            }
