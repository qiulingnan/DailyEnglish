...
extension Ranking:UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        number.textColor = UIColor(red: 100/255.0, green: 100/255.0, blue: 100/255.0, alpha: 1)
        cell.contentView.addSubview(number)
        _ = number.sd_layout()?.leftSpaceToView(cell.contentView,15)?.centerYEqualToView(cell.contentView)?.heightIs(30)
        number.setSingleLineAutoResizeWithMaxWidth(40)
        ...
        _ = name.sd_layout()?.leftSpaceToView(number,15)?.topSpaceToView(cell.contentView,10)?.heightIs(30)?.widthIs(150)
        
        let count = UILabel()
        count.text = "打赏次数:\(String(describing: reward.times!))"
        count.font = UIFont.systemFont(ofSize: 18)
        count.textColor = UIColor(red: 100/255.0, green: 100/255.0, blue: 100/255.0, alpha: 1)
        cell.contentView.addSubview(count)
        _ = count.sd_layout()?.leftSpaceToView(number,15)?.bottomSpaceToView(cell.contentView,10)?.heightIs(30)?.widthIs(150)
...
extension Ranking:JXPagingViewListViewDelegate {
